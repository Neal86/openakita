...
type Grant = { id: string; node_id: string; agent_profile_id: string; resource_id: string; fingerprint: string; remark: string; permissions: PermissionSet; sync_state?: string; sync_error?: string };
...
const defaultPermissions: PermissionSet = { read: true, screenshot: true, mouse: false, keyboard: false, launch: false, close: false };
...
function ResourceCard({ api, node, resource, profiles, reload }: { api: string; node: NodeInfo; resource: Resource; profiles: AgentProfile[]; reload: () => Promise<void> }) {
  ...
  const createGrant = async () => {
    ...
    try {
      ...
      if (data?.grant?.sync_state === "pending") toast.warning("授权已保存，远程设备恢复连接后会自动同步"); else toast.success("已授权给 Agent");
      setRemark(""); setPermissions({ ...defaultPermissions }); await reload();
  ...
  return <div className="space-y-3 rounded-lg border p-4">
    ...
    {resource.grants?.length > 0 && <div className="space-y-2">{resource.grants.map(grant => <div key={grant.id} className="rounded-md border bg-muted/20 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex flex-wrap items-center gap-2"><ShieldCheck size={15} /><span className="text-sm font-medium">{profiles.find(p => p.id === grant.agent_profile_id)?.name || grant.agent_profile_id}</span><span className="text-xs text-muted-foreground">{Object.entries(grant.permissions || {}).filter(([, enabled]) => enabled).map(([key]) => key).join(" · ")}</span>{grant.sync_state === "pending" && <Badge variant="secondary">待同步</Badge>}{grant.sync_error && <span className="text-xs text-destructive" title={grant.sync_error}>同步失败</span>}</div><Button size="sm" variant="ghost" onClick={() => void removeGrant(grant.id)}><Trash2 size={14} />取消授权</Button></div>
...
export function WindowsConnectorPanel({ apiBaseUrl = DEFAULT_API }: { apiBaseUrl?: string }) {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    } catch (error) { if (!quiet) toast.error(error instanceof Error ? error.message : String(error)); } finally { if (!quiet) setLoading(false); }
  }, [api]);
  ...
  const refreshNode = async (nodeId: string) => { const response = await safeFetch(`${api}/api/windows-connector/nodes/${encodeURIComponent(nodeId)}/refresh`, { method: "POST" }); if (!response.ok) { const data = await response.json().catch(() => ({})); toast.error(data.detail || "扫描失败"); return; } window.setTimeout(() => void load(true), nodeId === "local" ? 150 : 800); };
  ...
  return <div className="h-full overflow-y-auto p-5">
    ...
    <section className="mt-6 space-y-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">远程设备</h3><p className="text-xs text-muted-foreground">在其他 Windows 电脑上运行 Connector，并使用配对码接入当前 OpenAkita。</p></div><Button variant="outline" onClick={() => void download()} disabled={downloading}><Download size={14} />下载远程 Connector</Button></div>
      ...
      <div className="space-y-3">{remotes.map(renderNode)}{!remotes.length && <Card className="p-10 text-center text-sm text-muted-foreground">暂无远程 Windows 设备。下载 Connector 后使用上方配对码连接。</Card>}</div>
    </section>
  </div>;
}
