...
const PLATFORMS = [
  ...
  { id: "wechat_desktop", title: "微信（桌面版）", logo: null, docUrl: "" },
        ...
        <div className="min-w-0">
          <h3 className="flex min-w-max items-center gap-2 text-base font-bold tracking-tight">
            ...
            <Button
              ...
              onClick={async () => {
                const ok = await copyToClipboard("https://github.com/anthropic-lab/openakita/blob/main/docs/im-channels.md");
                if (ok) toast.success(t("config.imGuideDocCopied"));
              }}
              title={t("config.imGuideDoc")}
            >
              <BookOpen size={13} /><span className="hidden xl:inline">{t("config.imGuideDoc")}</span>
            </Button>
            <Button variant="outline" size="sm" className="h-7 shrink-0 gap-1 text-xs" onClick={() => setShowCmdRef(true)} title={t("config.imQuickCommands")}>
              <Terminal size={13} /><span className="hidden xl:inline">{t("config.imQuickCommands")}</span>
            </Button>
          </h3>
        ...
        {!wizardMode && (
          <div className="flex flex-col items-end gap-1 shrink-0">
            <label className={cn(
              ...
              chainPushOn ? "border-amber-400 bg-amber-50 dark:border-amber-600 dark:bg-amber-950/40" : "border-input bg-transparent",
            )}>
              <BrainCircuit size={16} className={cn(chainPushOn ? "text-amber-500" : "text-muted-foreground")} />
              <span className={cn("text-sm font-semibold", chainPushOn ? "text-amber-700 dark:text-amber-400" : "text-foreground")}>{t("config.imChainPush")}</span>
              <Switch checked={chainPushOn} onCheckedChange={(v) => setEnvDraft((d) => envSet(d, "IM_CHAIN_PUSH", String(v)))} />
              <span className={cn("text-sm w-8 font-semibold", chainPushOn ? "text-amber-600 dark:text-amber-400" : "text-muted-foreground")}>{chainPushOn ? "ON" : "OFF"}</span>
            </label>
            <span className="inline-flex items-center gap-1 text-[11px] text-amber-600 dark:text-amber-400"><AlertCircle size={12} className="shrink-0" />{t("config.imChainPushHelp")}</span>
          </div>
        )}
      </div>

      <div className="mt-4 space-y-2">
        <span className="text-sm font-bold text-foreground">{t("config.imPlatformOverview")}</span>
        <div className="flex flex-wrap gap-3">
          {PLATFORMS.map((p) => {
            ...
            return (
              <div key={p.id} className="flex items-center gap-2.5 rounded-lg border px-3 py-2.5 bg-card hover:bg-accent/50 transition-colors">
                {Logo ? <Logo size={28} /> : <MonitorSmartphone size={28} className="text-emerald-600" />}
                <span className="font-medium text-sm">{title}</span>
                {p.docUrl && (
                  <TooltipProvider delayDuration={200}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="text-muted-foreground hover:text-foreground transition-colors cursor-pointer" onClick={(e) => { e.stopPropagation(); openExternalUrl(p.docUrl); }}><ExternalLink size={13} /></span>
                      </TooltipTrigger>
                      <TooltipContent side="top" className="text-xs">{t("config.imDoc")} — {p.docUrl}</TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                )}
              </div>
...
const CMD_CATEGORIES: CmdCategory[] = [
  { labelKey: "imCmdCatTask", icon: <IconTarget size={14} />, entries: [
    { cmd: "停止 / stop / /stop / kill", desc: "imCmdStop" },
    { cmd: "跳过 / skip / /skip", desc: "imCmdSkip" },
    { cmd: "——", desc: "imCmdInsert" },
  ] },
  { labelKey: "imCmdCatChat", icon: <IconMessageCircle size={14} />, entries: [
    { cmd: "/new  /新话题", desc: "imCmdNew" }, { cmd: "/help  /帮助", desc: "imCmdHelp" },
  ] },
  { labelKey: "imCmdCatModel", icon: <IconBot size={14} />, entries: [
    { cmd: "/model", desc: "imCmdModel" }, { cmd: "/switch [name]", desc: "imCmdSwitch" }, { cmd: "/restore", desc: "imCmdRestore" },
  ] },
  { labelKey: "imCmdCatThinking", icon: <IconBrain size={14} />, entries: [
  ...
  { labelKey: "imCmdCatMode", icon: <IconWrench size={14} />, entries: [{ cmd: "/模式  /mode", desc: "imCmdMode" }] },
  { labelKey: "imCmdCatMultiAgent", icon: <IconUsers size={14} />, entries: [
    { cmd: "/切换  /switch", desc: "imCmdAgentSwitch" }, { cmd: "/状态  /status", desc: "imCmdAgentStatus" }, { cmd: "/重置  /agent_reset", desc: "imCmdAgentReset" },
  ] },
  { labelKey: "imCmdCatSystem", icon: <IconConfig size={14} />, entries: [{ cmd: "/restart  /重启", desc: "imCmdRestart" }] },
];
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Terminal size={18} />{t("config.imQuickCommandsTitle")}</DialogTitle>
          <DialogDescription>{t("config.imQuickCommandsDesc")}</DialogDescription>
        </DialogHeader>
        <div className="grid gap-3 mt-2">
          {CMD_CATEGORIES.map((cat) => (
            <div key={cat.labelKey} className="rounded-lg border bg-card px-4 py-3">
              <div className="text-sm font-semibold mb-2 flex items-center gap-1.5"><span className="inline-flex shrink-0 text-muted-foreground">{cat.icon}</span>{t(`config.${cat.labelKey}`)}</div>
              <div className="space-y-1.5">
                {cat.entries.map((e, i) => (
                  <div key={i} className="flex items-baseline gap-3 text-sm">
                    <code className="shrink-0 rounded bg-muted px-1.5 py-0.5 text-xs font-mono text-foreground">{e.cmd}</code>
                    <span className="text-muted-foreground text-xs">{t(`config.${e.desc}`)}</span>
                  </div>
                ))}
              </div>
            </div>
