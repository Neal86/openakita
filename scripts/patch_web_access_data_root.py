...
def main() -> None:
    ...
    auth_text = auth.read_text("utf-8")
    if "import secrets\nimport shutil\nimport threading\n" in auth_text:
        auth.write_text(
            auth_text.replace(
                "import secrets\nimport shutil\nimport threading\n",
    ...
    narrow_server = '''    try:\n        from openakita.config import settings\n\n        data_dir = Path(settings.project_root) / "data"\n    except Exception:\n        data_dir = Path.cwd() / "data"\n    web_access_config = WebAccessConfig(resolve_web_access_data_dir())\n'''
    broad_server = '''    data_dir = resolve_web_access_data_dir()\n    web_access_config = WebAccessConfig(data_dir)\n'''
    server_text = server.read_text("utf-8")
    if original_server in server_text:
        server.write_text(server_text.replace(original_server, narrow_server, 1), "utf-8")
    elif broad_server in server_text:
        server.write_text(server_text.replace(broad_server, narrow_server, 1), "utf-8")
    elif narrow_server not in server_text:
        raise SystemExit("server WebAccessConfig root marker changed")
