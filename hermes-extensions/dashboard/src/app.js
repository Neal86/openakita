(function () {
  ...
  function ManagementApp() {
    ...
    const loadHealth = useCallback(async function () {
      try { const health = await request("/wechat/health"); setWechatHealth(health); return health; }
    ...
    const loadCore = useCallback(async function (quiet) {
      ...
      try {
        ...
        const errors = [];
        if (results[0].status === "fulfilled") setManagement(results[0].value); else errors.push(errText(results[0].reason));
        if (results[1].status === "fulfilled") setWechatHealth(results[1].value); else errors.push(errText(results[1].reason));
        if (errors.length && !quiet) setError(errors.join(" · "));
      } finally { if (!quiet) setCoreLoading(false); }
    }, []);
    ...
    const loadTasks = useCallback(async function (quiet) {
      ...
      try {
        ...
        const results = await Promise.allSettled([request("/overview" + (q.toString() ? "?" + q.toString() : "")), request("/upcoming?" + uq.toString())]);
        const errors = [];
        if (results[0].status === "fulfilled") setTasks(results[0].value); else errors.push("Task list: " + errText(results[0].reason));
        if (results[1].status === "fulfilled") setUpcoming((results[1].value && results[1].value.items) || []); else errors.push("Upcoming: " + errText(results[1].reason));
        if (errors.length && !quiet) setError(errors.join(" · "));
        return results[0].status === "fulfilled" ? results[0].value : null;
      } finally { if (!quiet) setTasksLoading(false); }
    }, [taskProfile, taskRange, includeCompleted]);

    const checkWeChatDesktop = useCallback(async function () {
      setWechatLoading(true); setWechatErrors([]); setError("");
      const specs = [
        ["health", "/wechat/health"], ["desktop", "/wechat/status"], ["recent chats", "/wechat/chats?limit=40"], ["unread chats", "/wechat/unread?limit=40"]
      ];
      try {
        const results = await Promise.allSettled(specs.map(function (x) { return request(x[1]); }));
        const failures = [];
        results.forEach(function (r, i) {
          if (r.status === "rejected") { failures.push(specs[i][0] + ": " + errText(r.reason)); return; }
          if (i === 0) setWechatHealth(r.value);
          if (i === 1) setWechatStatus(r.value);
          if (i === 2) setWechatChats((r.value && r.value.items) || []);
          if (i === 3) setWechatUnread((r.value && r.value.items) || []);
        });
    ...
    const refreshCurrent = useCallback(async function () {
      setError("");
      if (tab === "tasks") { await Promise.all([loadCore(false), loadTasks(false)]); return; }
      if (tab === "wechat") { await loadHealth(); return; }
    ...
    useEffect(function () { if (tab === "wechat") loadHealth(); }, [tab, loadHealth]);
    useEffect(function () {
      const ms = tab === "wechat" ? 15000 : 30000;
      const timer = setInterval(function () {
        if (document.visibilityState !== "visible" || busy || anyDialog) return;
        if (tab === "tasks") { loadCore(true); loadTasks(true); }
        else if (tab === "wechat") loadHealth();
        else loadCore(true);
      }, ms);
      return function () { clearInterval(timer); };
    }, [tab, busy, anyDialog, loadCore, loadTasks, loadHealth]);

    function flattenTaskOverview(data) { return ((data && data.profiles) || []).flatMap(function (p) { return (p.cron || []).concat(p.kanban || []); }); }
    ...
    async function doAction(key, fn, success, after) {
      ...
      } catch (e) { setError(errText(e)); return { ok: false, error: e }; }
      finally { setBusy(""); }
    }
    ...
    async function createAgent(e) {
      ...
      if (!out.ok) return;
      setAgentModal(false); setAgentForm(Object.assign({}, DEFAULT_AGENT));
      if (out.result && out.result.agent) { setAgentDetail(out.result.agent); setAgentOriginal(agentEditable(out.result.agent)); }
    }
    async function openAgent(name) {
      setBusy("agent-open"); setError("");
      try { const data = await request("/agents/" + encodeURIComponent(name)); setAgentDetail(data); setAgentOriginal(agentEditable(data)); setTab("agents"); }
    ...
    async function saveAgent(e) {
      ...
      const out = await doAction("agent-save", function () { return request("/agents/" + encodeURIComponent(oldName), { method: "PATCH", body: JSON.stringify(payload) }); }, "Agent changes saved.");
      if (!out.ok) return;
    ...
    function agentAction(agent, action, value, label) {
      return doAction("agent:" + agent.name + ":" + action, function () { return request("/agents/" + encodeURIComponent(agent.name) + "/action", { method: "POST", body: JSON.stringify({ action: action, value: value || null }) }); }, label || "Agent action completed.", function (result) {
        if (result && result.agent && agentDetail && agentDetail.name === agent.name) setAgentDetail(Object.assign({}, agentDetail, result.agent));
      });
    }
    function restartAgent(agent) { askConfirm({ title: "Restart Gateway?", message: "Active sessions for " + agent.name + " may be interrupted.", confirmLabel: "Restart Gateway" }, function () { return agentAction(agent, "gateway_restart", null, "Gateway restarted."); }); }
    function deleteAgent(agent) { askConfirm({ title: "Delete Agent?", message: "This permanently deletes the Hermes Profile and its profile-scoped state.", detail: agent.name, confirmLabel: "Delete Agent", destructive: true }, async function () { const out = await doAction("agent-delete", function () { return request("/agents/" + encodeURIComponent(agent.name), { method: "DELETE" }); }, "Agent deleted."); if (out.ok) { setAgentDetail(null); setAgentOriginal(null); } }); }

    async function createProject(e) {
      e.preventDefault(); if (!projectSupported) return;
      ...
      if (!out.ok) return;
      setProjectModal(false); setProjectForm(Object.assign({}, DEFAULT_PROJECT));
      if (out.result && out.result.project) { setProjectDetail(out.result.project); setProjectOriginal(projectEditable(out.result.project)); }
    }
    async function openProject(project) {
      if (!projectSupported) return;
      setBusy("project-open"); setError("");
      try { const data = await request("/projects/" + encodeURIComponent(project.slug) + "?profile=" + encodeURIComponent(project.profile || "default")); setProjectDetail(data); setProjectOriginal(projectEditable(data)); setTab("projects"); }
      catch (e) { setError(errText(e)); } finally { setBusy(""); }
    }
    async function saveProject(e) {
      e.preventDefault(); if (!projectDetail) return;
      ...
      const out = await doAction("project-save", function () { return request("/projects/" + encodeURIComponent(projectDetail.slug), { method: "PATCH", body: JSON.stringify(payload) }); }, "Project changes saved.");
      if (out.ok) await openProject(projectDetail);
    }
    function projectAction(project, action, value, label) { return doAction("project:" + project.slug + ":" + action, function () { return request("/projects/" + encodeURIComponent(project.slug) + "/action", { method: "POST", body: JSON.stringify({ action: action, value: value || null, profile: project.profile || "default" }) }); }, label || "Project action completed.", function () { return openProject(project); }); }
    ...
    async function createTask(e) {
      ...
      const out = await doAction("task-create", function () { return request("/tasks", { method: "POST", body: JSON.stringify(payload) }); }, "Task created.");
      if (!out.ok) return;
      setTaskModal(false); setTaskForm(Object.assign({}, DEFAULT_TASK)); await loadTasks(true);
    }
    async function openTask(task) {
      ...
      try {
        ...
        if (!hasEditable && task && task.type && task.id) {
          const q = new URLSearchParams({ include_completed: "true" }); if (task.profile) q.set("profile", task.profile);
          const detailOverview = await request("/overview?" + q.toString());
          const candidate = flattenTaskOverview(detailOverview).find(function (row) { return row.id === task.id && row.type === task.type && (!task.profile || row.profile === task.profile); });
          if (candidate) actual = Object.assign({}, candidate);
        }
        setTaskDetail(actual); setTaskOriginal(taskEditable(actual));
        const hq = actual.profile ? "?profile=" + encodeURIComponent(actual.profile) + "&limit=40" : "?limit=40";
        const data = await request("/tasks/" + encodeURIComponent(actual.type) + "/" + encodeURIComponent(actual.id) + "/history" + hq); setHistory(data.items || []);
      } catch (e) { setError(errText(e)); setTaskDetail(Object.assign({}, task)); setTaskOriginal(taskEditable(task)); }
      finally { setHistoryLoading(false); }
    }
    async function saveTask(e) {
      e.preventDefault(); if (!taskDetail) return;
      const t = taskDetail; const payload = { name: t.name || "", prompt: t.prompt || t.body || "", profile: t.profile || "" };
      if (t.type === "cron") payload.schedule = t.schedule || ""; else payload.priority = Number(t.priority == null ? 50 : t.priority);
      const out = await doAction("task-save", function () { return request("/tasks/" + encodeURIComponent(t.type) + "/" + encodeURIComponent(t.id), { method: "PATCH", body: JSON.stringify(payload) }); }, "Task changes saved.");
      if (out.ok) { const next = Object.assign({}, t, payload); setTaskDetail(next); setTaskOriginal(taskEditable(next)); await loadTasks(true); }
    }
    function taskAction(task, action, value, label) {
      const execute = async function () {
        const out = await doAction("task:" + task.id + ":" + action, function () { return request("/tasks/" + encodeURIComponent(task.type) + "/" + encodeURIComponent(task.id) + "/action", { method: "POST", body: JSON.stringify({ action: action, value: value || null, profile: task.profile || null }) }); }, label || "Task action completed.");
        if (out.ok) {
          await loadTasks(true);
          if (action === "remove" || action === "archive") { setTaskDetail(null); setTaskOriginal(null); setHistory([]); }
            ...
            setTaskDetail(Object.assign({}, taskDetail, patch));
          }
        }
      };
    ...
    async function runWeChatDryRun(e) {
      e.preventDefault(); setBusy("wechat-dry-run"); setError(""); setNotice("");
      try { const result = await request("/wechat/dry-run", { method: "POST", body: JSON.stringify(wechatDryRun) }); setNotice(result && result.dry_run ? "Dry run completed. No message was sent." : "Dry run completed."); await loadHealth(); }
      catch (e2) { setError(errText(e2)); } finally { setBusy(""); }
    }

    const filteredAgents = agents.filter(function (a) { const q = agentSearch.trim().toLowerCase(); return !q || [a.name, a.display_name, a.description, a.workspace, a.model, a.provider].some(function (v) { return String(v || "").toLowerCase().includes(q); }); });
    const filteredProjects = projects.filter(function (p) { const q = projectSearch.trim().toLowerCase(); const ok = projectFilter === "all" || (projectFilter === "archived" ? p.archived : !p.archived); return ok && (!q || [p.name, p.slug, p.profile, p.primary_path, p.board].some(function (v) { return String(v || "").toLowerCase().includes(q); })); });
    const filteredTasks = taskRows.filter(function (t) { const q = taskSearch.trim().toLowerCase(); const ok = taskTypeFilter === "all" || t.type === taskTypeFilter; return ok && (!q || [t.name, t.id, t.schedule, t.status, t.profile].some(function (v) { return String(v || "").toLowerCase().includes(q); })); });
    ...
    function Header() {
      const refreshing = coreLoading || tasksLoading;
      return h(React.Fragment, null,
        h("div", { className: "hx-header" }, h("div", null, h("h1", null, "Hermes Management Center"), h("div", { className: "hx-muted" }, "Agents · Projects · Tasks · Windows WeChat")), h("div", { className: "hx-actions" }, h("button", { className: "hx-button secondary", disabled: refreshing || Boolean(busy), onClick: refreshCurrent }, refreshing ? "Refreshing…" : "Refresh"))),
    ...
    function Overview() {
      ...
      const c = (management && management.counts) || {}; const tc = (management && management.task_counts) || {}; const wh = wechatHealth || {};
    ...
    function Agents() {
      return h("div", { className: "hx-stack" }, h("div", { className: "hx-section-head" }, h("div", null, h("h2", null, "Agents"), h("div", { className: "hx-muted" }, "Hermes Profiles are isolated Agents.")), h("button", { className: "hx-button", type: "button", onClick: function () { setAgentModal(true); } }, "+ Create Agent")), h("div", { className: "hx-toolbar" }, h(SearchBox, { value: agentSearch, onChange: setAgentSearch, placeholder: "Search name, role, workspace, model…" }), h("span", { className: "hx-muted" }, filteredAgents.length + " of " + agents.length)), filteredAgents.length ? h("div", { className: "hx-agent-grid" }, filteredAgents.map(function (a) {
        const running = String(a.gateway || "").toLowerCase().startsWith("running");
        return h(Card, { key: a.name }, h("div", { className: "hx-agent-head" }, h("div", null, h("h2", null, a.display_name || a.name), h("div", { className: "hx-muted" }, a.name)), a.is_default ? h(Pill, { kind: "ok" }, "default") : h(Pill, { kind: running ? "ok" : "paused" }, a.gateway || "stopped")), h("p", { className: "hx-description" }, a.description || "No role description"), h("div", { className: "hx-kv" }, h("span", null, "Model"), h("strong", null, a.model || "not configured"), h("span", null, "Provider"), h("strong", null, a.provider || "—"), h("span", null, "Workspace"), h("strong", null, a.workspace || "—")), h("div", { className: "hx-actions" }, h("button", { className: "hx-button secondary", type: "button", onClick: function () { openAgent(a.name); } }, "Manage"), !a.is_default ? h("button", { className: "hx-button secondary", type: "button", disabled: Boolean(busy), onClick: function () { agentAction(a, "use", null, "Default Agent changed."); } }, "Set default") : null, h("button", { className: "hx-button secondary", type: "button", disabled: Boolean(busy), onClick: function () { agentAction(a, running ? "gateway_stop" : "gateway_start", null, running ? "Gateway stopped." : "Gateway started."); } }, busyIs("agent:" + a.name + ":" + (running ? "gateway_stop" : "gateway_start")) ? "Working…" : running ? "Stop" : "Start"), running ? h("button", { className: "hx-button secondary", type: "button", disabled: Boolean(busy), onClick: function () { restartAgent(a); } }, "Restart") : null));
