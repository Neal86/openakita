...
| _this commit_ | P-RC-9 P9.1e | test(parity/orgs): relax bb_concurrent_writes to corruption-parity (v1 has no lock; row_count differs by design) | +12/-9 (test_blackboard_parity.py rewrites the concurrent_writes branch + ledger +3) | 0 (8 parity still pass; flake eliminated) | ADR-0011 (parity gates the rewrite, not v1's racy semantics); ADR-0013 (strict v2 concurrency contract lives in test_blackboard_contract.py case 12) |
