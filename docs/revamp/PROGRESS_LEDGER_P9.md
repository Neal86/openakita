...
> **Sub-phase status (2026-05-19, P9.4a0 land)**: P9.0 closed, P9.1 closed (Nit-3 of 5 cleared; 4 ride to G-RC-9), P9.2 closed (parity 6/6, contract 36/36), P9.3 NodeScheduler closed (parity 4/4, contract 12/12, all 4 G-RC-9.2 nits folded in, no v1 touch). P9.4 OrgCommandService IN FLIGHT -- P9.4a0/a/b/b2 + P9.4c shipped (full v2 service + 10/10 parity green). Charter: contract suite + ADR-0013 wall-clock SLA + G-RC-9.4 across remaining 3 commits.
...
| _this commit_ | P-RC-9 P9.4c | test(parity/orgs): activate 10 command_service parity fixtures (xfail -> pass; request to_dict + 4 default_scope + 2 ForwardTarget + submit record shape) | +PLACEHOLDER (test_command_service_parity.py REPLACE 21-line xfail -> ~340 fixtures + ledger) | +10 / -1 xfail | ADR-0011 (subsystem decomposition); P-RC-9-PLAN section 5.2 OrgCommandService ignore set (command_id + timestamps); P-RC-9-PLAN section 5.1 (10 fixtures = max of any P9.x phase) |
