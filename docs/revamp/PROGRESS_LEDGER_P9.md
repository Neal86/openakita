...
| _this commit_ | P-RC-9 P9.1b2 | feat(runtime/orgs): add SqliteBlackboardBackend + get_default_blackboard_backend factory | +234 (blackboard.py +230 sqlite class + factory + sqlite3/contextmanager imports + __all__ + __init__.py +4) | 0 (deeper sqlite smoke run during commit prep) | ADR-0011 (Protocol-typed backend swap); ADR-0012 (cross-process safety unblocked) |
