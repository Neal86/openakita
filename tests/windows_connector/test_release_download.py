...
def test_release_download_validates_zip_before_caching(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENAKITA_WINDOWS_CONNECTOR_DOWNLOAD_URL", "https://example.invalid/connector.zip")
    monkeypatch.setattr(route.urllib.request, "urlopen", lambda *_args, **_kwargs: io.BytesIO(b"not-a-zip"))
...
def test_release_download_caches_valid_zip(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("OPENAKITA_WINDOWS_CONNECTOR_DOWNLOAD_URL", "https://example.invalid/connector.zip")
    payload = _zip_bytes()
    monkeypatch.setattr(route.urllib.request, "urlopen", lambda *_args, **_kwargs: io.BytesIO(payload))
